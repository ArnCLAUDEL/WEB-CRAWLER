...
public class ClientMessageHandler extends AbstractMessageHandler {
	...
	private final ServiceClientProtocolHandler serviceProtocolHandler;
	...
	public ClientMessageHandler(SerializerBuffer serializerBuffer, SocketAddress address, SessionClientProtocolHandler sessionProtocolHandler,
			...
			ServiceClientProtocolHandler serviceProtocolHandler) {
	...
	protected void handle() {
		synchronized (serializerBuffer) {
			...
			switch (flag) {
				case Flag.SERVICE_REQUEST: handleServiceRequest(); break;
				case Flag.SERVICE_OK: handleServiceOk(); break;
	...
	private void handleServiceRequest() {
		handleMessage(serializerBuffer, ServiceRequest.CREATOR, address, serviceProtocolHandler::handleServiceRequest);
	} 
	
	private void handleServiceOk() {
		handleMessage(serializerBuffer, ServiceOk.CREATOR, address, serviceProtocolHandler::handleServiceOk);
	}
